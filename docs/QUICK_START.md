...
Copy this entire prompt and paste into your agent:
...
After builder completes, copy this entire prompt and paste into your agent:
...
- All 8 steps show "STEP X COMPLETE"
- No "STEP X FAILED" messages

### Verifier Success:
- Output: "VERIFICATION COMPLETE: 10/10 checks passed"
- All checks show "CHECK X PASSED"
...
1. **Builder fails**: Note the STEP number, check AGENT_PROMPTS.md for that step's requirements
2. **Verifier fails**: Note the CHECK number, review what it couldn't "SHOW ME"
3. **Fix the issue** and re-run the failed agent
...
After successful build + verify, you should have:
...
├── install.sh
├── README.md (updated)
└── ~/Library/LaunchAgents/com.sandbox.snapshot.plist
```
